<template>
  <div class="animate-slide-up space-y-8 sm:space-y-12">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
      <NuxtLink v-for="(agency, index) in agencies" :key="agency.id" :to="`/agency/${agency.id}`"
        class="group relative focus:outline-none" :style="{ animationDelay: `${index * 0.1}s` }">
        <UCard :ui="{
          ...
          <template #header>
            <div class="flex items-start justify-between">
              <h2
                class="text-2xl font-bold text-slate-900 dark:text-white group-hover:text-primary-600 dark:group-hover:text-primary-400 transition-colors">
                {{ agency.name }}
              </h2>
              <div
                class="p-2.5 bg-primary-50 dark:bg-primary-500/10 rounded-xl text-primary-600 dark:text-primary-400 shrink-0 transform transition-transform group-hover:scale-110">
          ...
          <div class="grid grid-cols-2 gap-3 sm:gap-4 mt-2">
            <div
              class="bg-white dark:bg-slate-800/40 rounded-2xl p-4 border border-slate-100 dark:border-slate-700/50 shadow-sm flex flex-col justify-center">
              <span
                class="block text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-1.5">Available</span>
              <span class="text-3xl font-black text-primary-600 dark:text-primary-400">{{ agency.vacancies }}</span>
            </div>
            <div
              class="bg-white dark:bg-slate-800/40 rounded-2xl p-4 border border-slate-100 dark:border-slate-700/50 shadow-sm flex flex-col justify-center">
              <span
                class="block text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-1.5">Handled</span>
              <span class="text-3xl font-black text-slate-800 dark:text-slate-100">{{ agency.handledSituationsCount
              }}</span>
            </div>
          </div>
          ...
          <template #footer>
            <div
              class="flex items-center justify-between text-sm font-semibold text-slate-600 dark:text-slate-400 group-hover:text-primary-600 dark:group-hover:text-primary-400 transition-colors">
